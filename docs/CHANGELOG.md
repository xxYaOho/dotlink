...
- TUI 删除链接改为按模块分组的多选列表，并显示简洁健康状态
- doctor 输出改为与删除列表一致的 `🟢 / 🟡 / 🔴` 简洁状态提示
- 修复路径补全匹配过宽导致 `./docs/RE` 无法有效收敛的问题
- 路径补全显示动态进度（0-100%）并保持输入可继续
- `src` 路径模式改为随配置源切换：global 支持 `~/` / 绝对路径，local 使用项目内相对路径
- 目录扫描低于 `300ms` 时跳过 loading，减少快速补全时的闪烁感
- 慢扫描时改用 dot3 loading，并在单次 Tab 后直接展示候选列表
- 补充本地 `npm link` 安装时的可执行权限说明
